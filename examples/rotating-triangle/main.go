...
const (
	windowWidth  = 800
	windowHeight = 600
	windowTitle  = "go-webgpu: Rotating Triangle Example"
)
...
// Shader source (WGSL) with uniform buffer for transformation matrix
const shaderSource = `
struct Uniforms {
    transform: mat4x4f,
}
...
func (app *App) createWindow() error {
	className, err := windows.UTF16PtrFromString("GoWebGPURotatingTriangle")
	if err != nil {
		return err
	}
...
// renderTriangle encodes the rotating triangle rendering commands.
